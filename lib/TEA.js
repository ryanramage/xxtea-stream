...
Tea.bufferToLongs = function(buff) {  // convert string to array of longs, each containing 4 chars
    ...
    for (var i=0; i<l.length; i++) {
        ...
        var x = bops.readInt8(buff, i*4);
        try {  x+= (bops.readInt8(buff, i*4+1)<<8  ) } catch(ignore){}
        try {  x+= (bops.readInt8(buff, i*4+2)<<16 ) } catch(ignore){}
        try {  x+= (bops.readInt8(buff, i*4+3)<<24)  } catch(ignore){}

        l[i] = x;

    }
    console.log(buff.toString('utf-8'),l);
...
Tea.longToBuff = function(l) {  // convert array of longs back to string
    ...
    for (var i=0; i<l.length; i++) {
      console.log('write', l[i], i*4);
        bops.writeUInt32LE(a, l[i], i*4);
    }
